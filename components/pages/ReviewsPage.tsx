...
import PageHero from "../shared/Pagehero";
import Footer from "../Footer";
import WriteReviewModal from "../shared/WriteReviewModal";
...
const colors = ["bg-gold", "bg-gold-dark", "bg-gold-deeper"];
...
export default function ReviewsPage({ navigate }: ReviewsPageProps) {
    ...
    fetch("/api/reviews")
      ...
      .then((data) => {
        if (active && Array.isArray(data.reviews)) {
          setExtraReviews(
            data.reviews.map(
              (r: { name: string; service: string; stars: number; message: string }, i: number) => ({
                init: r.name.trim().charAt(0).toUpperCase() || "?",
                name: r.name,
                tag: r.service,
      ...
      .catch(() => {
        // Silently fall back to the default reviews if the request fails.
      });
    return () => {
      active = false;
    };
  ...
  // Newly submitted reviews appear first.
  const allReviews = [...extraReviews, ...reviews];
      ...
      <section className="py-[60px] px-[5%]">
        ...
        <div className="flex justify-center mb-8">
          <button
            ...
            className="flex items-center gap-2 text-[13px] px-6 py-2.5 rounded-full bg-gold hover:bg-gold-dark text-white transition-colors"
          >
      ...
      <WriteReviewModal
        ...
        onAdded={(review) =>
          setExtraReviews((prev) => [
            {
              init: review.name.trim().charAt(0).toUpperCase() || "?",
              name: review.name,
              tag: review.tag,
              stars: review.stars,
              text: review.text,
